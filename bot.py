    ...
    try:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            summary = await asyncio.get_event_loop().run_in_executor(
                None, lambda: generate_summary(transcript, user["groq_key"], user.get("taglish", False))
            )
...
async def transcribe_audio_file(file_path, tmp_dir, user, update, context, msg):
    converted = await asyncio.get_event_loop().run_in_executor(
        None, lambda: convert_to_mp3(file_path, tmp_dir)
    )
    chunks = await asyncio.get_event_loop().run_in_executor(
        None, lambda: chunk_audio(converted, tmp_dir)
    )
    total = len(chunks)
    parts = []
    for i, chunk_path in enumerate(chunks):
        ...
        text = await asyncio.get_event_loop().run_in_executor(
            None, lambda cp=chunk_path: transcribe_file(cp, user["groq_key"], user.get("taglish", False))
        )
        parts.append(text)
